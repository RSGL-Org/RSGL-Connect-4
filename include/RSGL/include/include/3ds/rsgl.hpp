...
namespace RSGL{
    ...
    struct window {
        ...
        window(C3D_RenderTarget* screen, RSGL::rect r, RSGL::color c) {
            ...
            RSGL::background_color = C2D_Color32(c.r, c.g, c.b, 0xFF); window::clr = c; RSGL::screen = screen;  window::x = r.x; window::y = r.y; window::w = r.width; window::h = r.length;
        };
        void close() {
			C3D_Fini();
 			gfxExit();
    ...
    bool window::isPressed(u32 key){return RSGL::isClicked(key);}
    ...
    void window::checkEvents() {
        ...
        if (RSGL::isClicked(KEY_TOUCH)) event.type=RSGL::MouseButtonPressed;
        else if (RSGL::isReleased(KEY_TOUCH)) event.type=RSGL::MouseButtonReleased;
    }
